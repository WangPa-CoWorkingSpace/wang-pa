<template>
  <div id="navbar" class="fixed z-40 w-full transition-all duration-[1200ms]"
    ...
    <div class="flex justify-between min-h-[60px] items-center px-2 border-1">
      ...
      <nav class="items-center gap-[30px] font-normal hidden lg:flex"
        ...
        <div>
          <NuxtLink @click.prevent="logoutBTN"
            :class="{ 'hidden': !user_email_CK || !user_avatar_CK || !user_full_name_CK }">Logout</NuxtLink>
        </div>
      </nav>
    ...
    <div id="mobileMenu"
      class="lg:hidden justify-center space-y-2 text-center transition-all duration-200 overflow-hidden"
      ...
      <div>
        <NuxtLink @click.prevent="logoutBTN"
          :class="{ 'hidden': !user_email_CK || !user_avatar_CK || !user_full_name_CK }">
          Logout</NuxtLink>
      </div>
    </div>
  </div>
